...
#define L 0x04
#define T 0x00
#define BLACK_BLACK ((BLACK<<4)|BLACK)
...
const fixed nMapSize = 16; // 1024
const fixed xMapSize = (16 << 10); // 1024

const unsigned int uScreenWidth = 64u;
const unsigned int uScreenHeight = 48u;
...
fixed 	xFarX1,
		...
		xWorldA,
		xFoVHalf;
ufixed uxSampleDepth,
		uxSampleWidth;
ufixed *uxSampleDepthY,
	    *uxSampleWidthX;
fixed 	xStartX,
       	xStartY,
	   	xEndX,
...
{
	ufixed uxTopScreenWidth = uint2ufixed(uScreenWidth-1u);
	ufixed uxTopHalfScreenHeight = uint2ufixed((uScreenHeight/2u)-1u);

	uxSampleDepthY = MMalloc(uHalfScreenHeight*sizeof(ufixed));
	for (y = 0; y < uHalfScreenHeight; y++)
	{
		uxSampleDepthY[y] = ufixed_div(uint2ufixed(y), uxTopHalfScreenHeight);	
	}

	uxSampleWidthX = MMalloc(uScreenWidth*sizeof(ufixed));
	for (x = 0; x < uScreenWidth; x++)
	{
		uxSampleWidthX[x] = ufixed_div(uint2ufixed(x), uxTopScreenWidth);
	}
}
...
int temp;
int nScreenMemSize = 32*48;
int nHalfScreenMemSize = 32*24;

int main()
{
	uHalfScreenHeight = uScreenHeight/2u;
	xWorldX = float2fixed(fWorldX);
	xWorldY = float2fixed(fWorldY);
	...
	while (1)
	{
		...
		temp = fixed_div(xWorldA + xFoVHalf, xTWO_PI) << BITS_DIFF;
		xSinPlus = nsin(temp) >> BITS_DIFF;
		xCosPlus = ncos(temp) >> BITS_DIFF;
		temp = fixed_div(xWorldA - xFoVHalf, xTWO_PI) << BITS_DIFF;
		xSinMinus = nsin(temp) >> BITS_DIFF;
		xCosMinus = ncos(temp) >> BITS_DIFF;
		...
		MemFill(vbuffer + nHalfScreenMemSize, BLACK_BLACK, nHalfScreenMemSize);
		...
		for (y = 0; y < uHalfScreenHeight; y++)
		{
			...
			uxSampleDepth = uxSampleDepthY[y];
			
			// Use sample point in non-linear (1/x) way to enable perspective
			// and grab start and end points for lines across the screen
			xStartX = fixed_div(xFarX1, uxSampleDepth);
			xStartY = fixed_div(xFarY1, uxSampleDepth);
			xEndX = fixed_div(xFarX2, uxSampleDepth);
			xEndY = fixed_div(xFarY2, uxSampleDepth);

			// Linearly interpolate lines across the screen
			for (x = 0; x < uScreenWidth; x++)
			{
				uxSampleWidth = uxSampleWidthX[x];
				...
				// Sample symbol and colour from map sprite, and draw the pixel to the screen
				xSampleX = fixed_mul(uxSampleWidth, xEndX-xStartX) + xStartX + xWorldX;
				if (xSampleX>=0 && xSampleX < xMapSize)
				{
					xSampleY = fixed_mul(uxSampleWidth, xEndY-xStartY) + xStartY + xWorldY;
					if (xSampleY>=0 && xSampleY < xMapSize)
					{
						Point(x, y+uHalfScreenHeight, sprGround[nMapSize*fixed2int(xSampleY) + fixed2int(xSampleX)]);		
        ...
        while (bInvalidKey)
        {
			...
			else if (cKeyPressed == 0x20)
            {
                ...
                Screen(0);
                Width(40);
                x =print(0, 0, "X=");     x+=printFloat(x, 0, fixed2float(xWorldX));
                x+=print(x, 0, ", Y=");   x+=printFloat(x, 0, fixed2float(xWorldY));
            ...
            else
            {
                ...
                if (cKeyPressed == 1 || cKeyPressed == 2 || cKeyPressed == 8) 
                {
                    ...
                    xWorldX += xcos(xWorldA);
					xWorldY += xsin(xWorldA);
                }
