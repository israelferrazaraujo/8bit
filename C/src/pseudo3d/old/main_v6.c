...
const unsigned int uScreenWidth = 64u,	\   // Screen Resolution X (columns)
                   ...
                   uWindowWidth = 64u,	\   // Window Screen Size X (columns)
                   uWindowHeight = 48u, \    // Window Screen Size Y (rows)
...
int *nRayAngles, \     // Ray Angle Lookup Table
     nStepAngle;
...
int main()
{
    ...
    nStepAngle = (int)(fStepAngle*fFIXED*10);
    // Create Ray Angle Lookup Table
    nRayAngles = MMalloc(2*uWindowWidth);    
    for (int l=0; l<=uWindowWidth; l++)           
    {
        nRayAngles[l] = (int)((fHalfFOV - (l/fWindowWidth)*fFOV)*fFIXED*10);
    }
    ...
    while (1)
	{
        ...
        nPlayerA = (int)(fPlayerA*fFIXED*10);
        x = uWindowWidth;
        while (x--)
		{
            //Cls();
			...
			uFloor = uWindowHeight - uCeiling -1u + uWindowY;
            uPosX = x + uWindowX;
            //PutText(4*28, 4*30, "4", 0);
            if (uCeiling > uWindowY)
            {
                line(uPosX, uWindowY, uPosX, uCeiling, BLACK);
                line(uPosX, uFloor, uPosX, uWindowHeight-1u, GRAY);
            }
            line(uPosX, uCeiling, uPosX, uFloor, cColor1);
